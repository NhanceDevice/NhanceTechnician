...
/**
 * Created by Javeed on 2/10/2016.
 */
public class TimeDiff {

//    http://technojeeves.com/index.php/37-difference-between-two-dates-in-java
    ...
    public static void main(String[] args) {
        ...
        System.out.printf("Just the number of days = %d\n",
                TimeDiff.getTimeDifference(d0, d1, TimeField.DAY));
    }

    public static String getTimeDifferenceAsString(Date d0, Date d1) {
        ...
        if(diff[0] > 0)
        {
            stringBuilder.append(Long.toString(diff[0])+" day ago");
        }
        else if(diff[1] > 0)
        {
            stringBuilder.append(Long.toString(diff[1])+" hrs ago");
        }
        else if(diff[2] > 0)
        {
            stringBuilder.append(Long.toString(diff[2])+" mins ago");
        }
        else if(diff[3] > 0)
        {
            //stringBuilder.append(Long.toString(diff[3])+" secs ago");
            stringBuilder.append("a min ago");
        }

        return timeDifference = stringBuilder.toString();
    }
