...
public class JSONUtil {
	...
	/**
	 * Given a JSONObject and a key path (e.g property.child) and a default it will traverse the object graph and pull out the desired property
	 */
	...
	public static <U> U queryJSON(JSONObject source, String query, U defaultObject) {
		...
		if (nextSeperator == -1 && nextIndexStart == -1) {
			...
			try {
				...
				LOG.e(TAG, "Unable to cast the object to " + defaultObject.getClass().getName(), e);
				return defaultObject;
			} catch (JSONException e) {
				LOG.e(TAG, "Unable to get the Key from the input object. Key:" + query, e);
				return defaultObject;
			}
		}
		int endQuery;
		...
		try {
			...
			if (result.getClass().isAssignableFrom(defaultObject.getClass())) {
				...
				LOG.w(
						TAG,
						String.format("The returned object type %s is not assignable to the type %s. Using default!", result.getClass(),
	...
	/**
	 * Given a JSONArray and a query (e.g. [0].property) it will traverse the array and pull out the requested property.
	 *
	 * Acceptable indexes include negative numbers to reference items from the end of the list as well as "last" and "first" as more explicit references to
	 * "0" and "-1"
	 */
	@SuppressWarnings("unchecked")
	public static <U> U queryJSON(JSONArray source, String query, U defaultObject) {
		...
		try {
			if (remainingQuery.indexOf(QUERY_ARRAY_INDEX_START) == 0) {
				...
				LOG.w(TAG, String.format("Incorrect query for next object %s", remainingQuery));
				return defaultObject;
			}
	...
	/*
	 * replacement for JSONObject.optBoolean() - optBoolean() only checks for "true" and "false", but our API sometimes uses "0" to denote false
	 */
	...
	/*
	 * returns the JSONObject child of the passed parent that matches the passed query this is basically an "optJSONObject" that supports nested queries,
	 * for example:
	 * 
	 ...
	 * would find this:
	 * 
	 * "meta": { "data": { "site": { "ID": 3584907, "name": "WordPress.com News", } } }
	 */
