...
public class DialogPlusBuilder {
   ...
   */
  public DialogPlus create(boolean isTablet) {
    init();
    return new DialogPlus(this, isTablet);
  }
  ...
  public int[] getContentMargin(boolean isTablet) {
    Activity activity = (Activity) context;
    Display display = activity.getWindowManager().getDefaultDisplay();
    ...
    if(!isTablet)
    {
      int minimumMargin = isTablet == true ? displayWidth/4 : context.getResources().getDimensionPixelSize(R.dimen.default_center_margin);
      for (int i = 0; i < margin.length; i++) {
        margin[i] = getMargin(this.gravity, margin[i], minimumMargin);
      }
    }
    ...
    {
      for (int i = 0; i < margin.length; i++) {
        int minimumMargin = i%2 == 0 ? displayWidth/4 : context.getResources().getDimensionPixelSize(R.dimen.default_center_margin_tab);
        margin[i] = getMargin(this.gravity, margin[i], minimumMargin);
      }
    }
